...
export class DialogService {
  ...
  constructor(
    ...
    const initialData: any = localStorage.getItem('dialog_data')
  ...
  open(component: Type<any>, config: ChildConfig) {
    ...
    if (config.position) {
      const element: HTMLElement | any = this.document.getElementById(this.dialogElementId);
  ...
  saveData(data: object | null) {
    localStorage.setItem('dialog_data', JSON.stringify(data))
    this.data$.next(data);
  }
